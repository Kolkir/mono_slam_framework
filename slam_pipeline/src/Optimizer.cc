...
void Optimizer::BundleAdjustment(const std::vector<KeyFrame*>& vpKFs,
  ...
  for (size_t i = 0; i < vpMP.size(); i++) {
    ...
    for (auto mit = observations.begin(); mit != observations.end(); mit++) {
      ...
      e->setVertex(0, v0);
      auto v1 = optimizer.vertex(pKF->mnId);
      e->setVertex(1, v1);
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag,
  ...
  for (std::list<KeyFrame*>::iterator lit = lLocalKeyFrames.begin(),
    ...
    KeyFrame* kf = *lit;
    g2o::VertexSE3Expmap* vSE3 =
        static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(kf->mnId));
    g2o::SE3Quat SE3quat = vSE3->estimate();
...
int Optimizer::OptimizeSim3(MatchFramesResult& vpMatches1, g2o::Sim3& g2oS12,
  ...
  // Calibration
  const cv::Mat& K1 = vpMatches1.pF1->mK;
  const cv::Mat& K2 = vpMatches1.pF2->mK;

  // Camera poses
  const cv::Mat R1w = vpMatches1.pF1->GetRotation();
  const cv::Mat t1w = vpMatches1.pF1->GetTranslation();
  const cv::Mat R2w = vpMatches1.pF2->GetRotation();
  const cv::Mat t2w = vpMatches1.pF2->GetTranslation();
  ...
  for (int i = 0; i < N; i++) {
    MapPoint* pMP1 = vpMatches1.GetMapPoint1(i);
    MapPoint* pMP2 = vpMatches1.GetMapPoint2(i);
    ...
    if (pMP1 && pMP2) {
      if (!pMP1->isBad() && !pMP2->isBad() &&
          pMP2->GetKeyPointInKeyFrame(static_cast<KeyFrame*>(vpMatches1.pF2),
                                      i2)) {
    ...
    const cv::Point2i& kpUn1 = vpMatches1.keyPoints1[i];
    obs1 << kpUn1.x, kpUn1.y;
