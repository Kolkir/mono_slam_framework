...
Initializer::Initializer(const cv::Mat &K, float sigma, int iterations) {
  K.copyTo(mK);
...
bool Initializer::InitializeOpenCV(const MatchFramesResult &matchResult,
  ...
  cv::Mat E, inlierMask;
  E = cv::findEssentialMat(mvKeys1, mvKeys2, mK, cv::RANSAC, 0.999, 1.0,
                           inlierMask);
  if (!E.empty()) {
    double distanceThresh = 5;
    cv::Mat points3d;
    ...
    if (nInliners > minTriangulaed) {
      for (unsigned i = 0; i < mvKeys1.size(); i++) {
        if (inlierMask.at<uchar>(i)) {
          ...
          vP3D[i].x = static_cast<float>(points3d.at<double>(0, i) / w);
          vP3D[i].y = static_cast<float>(points3d.at<double>(1, i) / w);
          vP3D[i].z = static_cast<float>(points3d.at<double>(2, i) / w);
        }
      }
...
bool Initializer::Initialize(const MatchFramesResult &matchResult, cv::Mat &R21,
  ...
  for (int it = 0; it < mMaxIterations; it++) {
    ...
    for (size_t j = 0; j < 8; j++) {
      std::uniform_int_distribution<size_t> distr(0,
                                                  vAvailableIndices.size() - 1);
      size_t randi = distr(random_engine);
  ...
  std::thread threadH(&Initializer::FindHomography, this,
                      ref(vbMatchesInliersH), std::ref(SH), std::ref(H));
  std::thread threadF(&Initializer::FindFundamental, this,
                      ref(vbMatchesInliersF), std::ref(SF), std::ref(F));
...
void Initializer::FindFundamental(std::vector<bool> &vbMatchesInliers,
  ...
  // Number of putative matches
  const size_t N = vbMatchesInliers.size();
...
float Initializer::CheckHomography(const cv::Mat &H21, const cv::Mat &H12,
  ...
  for (int i = 0; i < N; i++) {
    ...
    const cv::Point2f &kp1 = mvKeys1[mvMatches12[i].first];
    const cv::Point2f &kp2 = mvKeys2[mvMatches12[i].second];
  ...
  for (size_t i = 0; i < 8; i++) {
    ...
    int nGood =
        CheckRT(vR[i], vt[i], mvKeys1, mvKeys2, mvMatches12, vbMatchesInliers,
                K, vP3Di, 4.0f * mSigma2, vbTriangulatedi, parallaxi);
...
void Initializer::Normalize(const std::vector<cv::Point2f> &vKeys,
...
int Initializer::CheckRT(const cv::Mat &R, const cv::Mat &t,
  ...
  for (size_t i = 0, iend = vMatches12.size(); i < iend; i++) {
    ...
    const cv::Point2f &kp1 = vKeys1[vMatches12[i].first];
    const cv::Point2f &kp2 = vKeys2[vMatches12[i].second];
    cv::Mat p3dC1;
    ...
    auto dist1 = cv::norm(normal1);

    cv::Mat normal2 = p3dC1 - O2;
    auto dist2 = cv::norm(normal2);

    auto cosParallax = normal1.dot(normal2) / (dist1 * dist2);
    ...
    float squareError1 =
        (im1x - kp1.x) * (im1x - kp1.x) + (im1y - kp1.y) * (im1y - kp1.y);
    ...
    float squareError2 =
        (im2x - kp2.x) * (im2x - kp2.x) + (im2y - kp2.y) * (im2y - kp2.y);
  ...
  if (nGood > 0) {
    ...
    size_t idx = std::min(size_t(50), vCosParallax.size() - 1);
